...
- If you disable the Caddy installation the `reverse_proxy` Docker network is still created. So we don't have to add different compose files. And if you want to use a different reverse proxy inside a Docker container you can use this network nevertheless.
