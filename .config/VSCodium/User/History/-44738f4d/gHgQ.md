...
You can do this with kustomize and add this snippet to the patches section. The existing `*.patch.yaml` file templates can be found [here](./examples/patches/).
```yaml
patches:
  [...]
  - path: mariadb.patch.yaml # this has to be the name of the *.patch.yaml file
    target:
      kind: StatefulSet # this has to be identical to the content of the *.patch.yaml file
      name: mariadb # this has to be identical to the content of the *.patch.yaml file
```
