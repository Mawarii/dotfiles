data "template_file" "cilium-values" {
  template = file("helm/cilium/values.yaml")
  vars = {
...
data "template_file" "hcloud-csi-values" {
  template = file("helm/hcloud-csi/values.yaml")
  vars = {
    cluster_address = hcloud_rdns.rdns-lb.dns_ptr
  }
...
resource "helm_release" "hcloud-csi" {
  ...
  values = [data.template_file.hcloud-csi-values.rendered]
