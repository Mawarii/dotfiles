...
func (r *MetricsServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if len(metricsServer.Spec.DeploymentSuffix) == 0 {
		id := util.GenerateRandomID(6)
		metricsServer.Spec.DeploymentSuffix = id
	}

	// deployment := v1.Deployment(metricsServer.Name, r.Namespace)

	deployment := &v1apps.Deployment{
		ObjectMeta: v1meta.ObjectMeta{
			Name:      fmt.Sprintf("metrics-server-%s", metricsServer.Spec.DeploymentSuffix),
			...
			Template: v1.PodTemplateSpec{
				...
				Spec: v1.PodSpec{
					Containers: []v1.Container{
						{
							Name:  "metrics-server",
							Image: metricsServer.Spec.Image,
						},
					},
				},
			},
