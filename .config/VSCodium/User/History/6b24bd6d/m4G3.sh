...
if [ "$BACKUP_ENVIRONMENT" = "generic-local" ]; then
...
if [ "$ENABLE_RETENTION" = true ]; then
    ...
    echo "$BACKUP_PATH"
    $MCLI find "$BACKUP_PATH" -name "*.sql.gz" --older-than +$RETENTION_DAYS -exec rm {} \; || fail "couldn't delete old backups. check your permissions."
fi
