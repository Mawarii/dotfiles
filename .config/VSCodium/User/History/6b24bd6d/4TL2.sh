...
if [ "$DISABLE_RETENTION" == false ]; then
    printf "Deleting backups older than $RETENTION_DAYS days...\n"

    $MCLI find "$BACKUP_PATH" -name "*.sql.gz" --older-than +"${RETENTION_DAYS}d" -exec "$MCLI rm {}" || fail "couldn't delete old backups. check your permissions."
fi
