...
This version upgrades Loki to 6.7.1 with a lot of changes. The important one for this helm chart are `schema configs`. You have to provide one or else the helm chart will fail with a warning. We added a test schema, but this is for dev purposes only.
