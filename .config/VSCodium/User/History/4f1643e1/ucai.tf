...
variable "domain_project" {
  description = "Project Base Domain"
}
...
module "kube-hetzner" {
  ...
  # * Your ssh public key
  ssh_public_key = file("${path.module}/ssh/wsp_k3s_ed25519_key.pub")
  ...
  ssh_private_key = file("${path.module}/ssh/wsp_k3s_ed25519_key")
  ...
  agent_nodepools = [
    ...
    {
      name        = "worker-mid", #! use worker-medium instead
      server_type = "cpx31",
      location    = "fsn1",
      labels      = [],
      taints      = [],
  ...
  base_domain = var.domain_cluster
  ...
  extra_firewall_rules = [
    ...
    # },
    # {
    #   description = "Allow Gitlab-Runner access to container registry"
    #   direction       = "out"
    #   protocol        = "tcp"
    #   port            = "5555"
    #   source_ips      = [] # Won't be used for this rule
  ...
  # Placement groups are a neat feature but they are limited to 500 nodes per project, if you need to get past that limit, you can disable it.
  ...
  lb_hostname = var.domain_cluster
  ...
  # rancher_hostname = var.domain_cluster
  ...
  # (Can be used by another rancher2 to continue setup of rancher outside this module.)
...
resource "local_file" "write_kubeconfig" {
  depends_on = [module.kube-hetzner]
  content    = module.kube-hetzner.kubeconfig
  filename   = "${path.module}/${var.cluster_name}_kubeconfig.yaml"
}

resource "null_resource" "dependency_trigger" {
  triggers = {
    kube_hetzner_complete = module.kube-hetzner.kubeconfig_file
  }
}

data "hcloud_load_balancer" "lb_primary" {
  depends_on = [module.kube-hetzner]
...
resource "hetznerdns_record" "primary" {
  ...
  name    = "idp"
  value   = data.hcloud_load_balancer.lb_primary.ipv4
  type    = "A"
  ttl     = 60
}

resource "hetznerdns_record" "primary_wildcard" {
  zone_id = data.hetznerdns_zone.dns_zone.id
  name    = "*.idp"
  value   = data.hcloud_load_balancer.lb_primary.ipv4
  type    = "A"
...
module "custom_modules" {
  ...
  domain_project             = var.domain_project
