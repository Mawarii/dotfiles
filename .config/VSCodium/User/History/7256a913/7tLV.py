...
while True:
    try:
        text = input("\nGib einen Satz ein (oder 'quit' zum Beenden): ").strip().lower()
        if text == 'clear' or text == 'c':
            ...
            for word in inputlist:
                if word in words:
                    translation.append(bcolors.OKGREEN + words[word] + bcolors.ENDC)
                else:
                    translation.append(bcolors.FAIL + word + bcolors.ENDC)
            print(' '.join(translation))
    except Exception as e:
        print(bcolors.FAIL + "Fehler: " + str(e) + bcolors.ENDC)
