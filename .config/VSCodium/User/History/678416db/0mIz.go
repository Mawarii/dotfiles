...
type ReqCharacter struct {
	...
	Skills       []model.CharacterSkillCategory `json:"skills,omitempty"`
}
...
func CreateCharacter(c *fiber.Ctx) error {
	...
	if body.Skills != nil {
		for _, skill := range body.Skills {
			createCharacterSkillCategory(character.ID, skill.SkillID, skill.SkillCategoryID)
		}
	}
...
func UpdateCharacter(c *fiber.Ctx) error {
	...
	var skills []model.CharacterSkillCategory

	if body.Skills != nil {
		for _, skill := range body.Skills {
			newSkill, err := createCharacterSkillCategory(character.ID, skill.SkillID, skill.SkillCategoryID)
			if err != nil {
				return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func createCharacterSkillCategory(characterID uint, skillID uint, categoryID uint) (model.CharacterSkillCategory, error) {

	return model.CharacterSkillCategory{}, nil
}
