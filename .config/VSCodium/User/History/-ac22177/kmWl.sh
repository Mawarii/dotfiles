...
if [[ "$ANSWER" = "y" || "$ANSWER" = "Y" ]]; then  
  ...
  if [[ $? != 0 ]]; then
    echo "Aborting..."
    echo "Please add $REPO_URL to your /etc/hosts file"
    exit 1
  fi
  ...
  kubectl apply -n "$NAMESPACE" -f "resources/$EAKTE_CERTIFICATE"
  kubectl apply -n "$NAMESPACE" -f "resources/$EAKTE_SECRET"
  kubectl apply -n "$NAMESPACE" -f "resources/$EAKTE_CONNECTOR_CONFIG"
  kubectl apply -n "$NAMESPACE" -f "resources/$EAKTE_CONNECTOR_ENV"
  kubectl apply -n "$NAMESPACE" -f "resources/$EAKTE_CONNECTOR"
